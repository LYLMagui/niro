...
from utils.proxy_helper import get_random_proxy
...
class BuffSpider:
    ...
    def get_goods_list(self, goods_id, page_num=1):
        ...
        try:
            ...
            proxy_info = f" 使用代理: {proxies['http']}" if proxies else " 未使用代理"
            
            logger.info(f"正在爬取 goods_id={goods_id}, page={page_num} ...{proxy_info}")
