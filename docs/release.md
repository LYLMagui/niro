...
- [功能] **分类抓取逻辑深度优化与 Redis 暂存机制实现**：
  ...
  - **探测与清理**：利用临时探测脚本 [test_api_tags.py] 完成了对 BUFF 接口标签结构的实时探测，输出详情后已按规范清理所有测试代码，确保生产环境纯净。
...
- [优化] **任务扫描频率安全加固**：
  ...
  - **爬虫端 (Python)**：
    - 同步更新了 `BuffScanTaskDTO` 和 `task_scanner.py` 中的扫描间隔默认值与兜底逻辑，统一设为 15 秒。
    - 优化了调度器的休眠日志输出，确保与实际频率一致。
  - **前端交互 (UX)**：
    - 重构了 [TaskConfig.vue](file:///e:/CodeSpace/PYTHON/niro/niro-client/src/views/TaskConfig.vue) 的相关逻辑，将 `scanInterval` 的输入下限硬性约束为 15。
    - 同步更新了新增任务、编辑任务及从商品列表快捷创建任务时的默认值初始化。
    - 优化了 UI 提示信息，明确告知用户 15 秒的频率下限及其背后的安全考量。
...
- [优化] **扫描监控日志全链路 IP 追踪**：
  ...
  - **日志一致性增强**：实现了扫描任务从“规则匹配”到“模拟购买”再到“进度更新”的全链路日志前缀统一（格式：`ip：xxx | ...`），大幅提升了在大规模代理切换场景下的可追溯性。

## v1.14.5 (2026-01-09)
- [优化] **监控日志层级与网络透明度重构**：
  - **日志分层展示**：任务入口日志精简为“正在执行任务”，将复杂的商品 ID、商品名及筛选参数下沉至具体的“扫货规则/盈利标准”日志中，实现从宏观到微观的平滑过渡。
  - **IP 信息深度整合**：移除了原有的独立 IP 打印日志，将当前的出口 IP 信息直接整合到每轮扫描结束的“休眠提示”中（格式：`ip：xxx | 💤 扫描完成...`），在保持控制台整洁的同时，确保护航网络环境的实时透明。

## v1.14.4 (2026-01-09)
- [优化] **监控日志交互体验升级**：
  - **状态反馈闭环**：在扫描周期增加 `😴 未发现符合条件的商品` 提示，避免在未命中目标时出现日志真空，让用户明确系统正在运行。
  - **休眠节奏可视化**：在每轮扫描结束时打印 `💤 扫描完成，休眠 Xs 后开始下轮...`，清晰展示任务调度的实时节奏。
  - **文案情感化修饰**：将枯燥的 `[炼金参数]` 替换为更有代入感的 `🎯 [扫货规则]`，将 `[倒卖参数]` 替换为更具商业感的 `📈 [盈利标准]`。

## v1.14.3 (2026-01-08)
- [优化] **扫描监控日志精简与文案校准**：
  - **文案语义化**：将 `task_scanner` 中的扫描启动日志从“启动扫描”修正为“正在扫描”，更准确地描述循环任务的实时运行状态。
  - **日志去重 (Noise Reduction)**：隐藏了 `buff_spider` 中底层的“正在爬取...”调试级日志，减少了高频扫描时的控制台干扰，使业务层日志（参数展示、发现目标）更突出。

## v1.14.2 (2026-01-08)
- [修复] **Buff 响应解析鲁棒性增强 (Pydantic 校验加固)**：
  - **UID 类型兼容**：将 `BuffSellOrderItem` 和 `ParsedBuffItemDTO` 中的 `user_id` 类型从 `int` 更改为 `str`。完美兼容 Buff API 返回的带 'U' 前缀的字符串 ID，解决了 `int_parsing` 导致的校验中断。
  - **字段缺失防御**：将 `BuffGoodsInfo` 中的 `buy_max_price` 和 `sell_num` 设为 `Optional` 并提供默认值。解决了部分饰品（如新上架或特殊道具）缺失上述字段导致的 `Field required` 异常，确保扫描任务在大规模数据抓取时的连续性。
  - **验证通过**：经实测，炼金扫货任务（ID: 8）在修复后能够正常解析 goods_id=33877 的响应数据，日志无报错，系统运行平稳。

## v1.14.1 (2026-01-08)
- [测试] **部署前支付逻辑加固与模拟增强**：
  - **购买逻辑管控**：确认并强化了 `task_scanner.py` 中的支付逻辑注释状态，确保在服务器部署测试期间绝不会触发真实资金扣除。
  - **模拟支付闭环**：优化了 `buy_goods` 方法的模拟返回结果，将 `state_text` 从“待支付”调整为“支付成功 (模拟测试)”。
  - **通知系统联动**：通过模拟“支付成功”状态，触发购买通知系统展示“✅ 购买成功”的样式，为用户提供完整的流程闭环测试体验。

## v1.14.0 (2026-01-08)
- [功能] **全量上线“印花溢价捡漏”核心引擎**：
  - **深度解析 (Crawler)**：重构了 `BuffSpider` 和 `BuffSellOrderItem` 模型，支持对饰品 `asset_info` 中嵌套贴纸信息的深度提取与结构化解析。
  - **估值模型 (Calculator)**：新增 `PremiumCalculator` 工具类，内置多级印花溢价算法，支持基于印花底价、磨损惩罚、稀有度权重的理论溢价计算。
  - **扫描器集成 (Task Engine)**：在 `task_scanner` 的炼金模式中无缝集成印花捡漏逻辑。系统在扫描基础磨损的同时，会自动评估饰品携带印花的附加价值，实现“低价高配”饰品的自动识别与预警。
  - **通知增强 (UX)**：优化了购买通知模板，当触发印花捡漏时，会自动在通知中展示印花总价、理论溢价及实际溢价对比，提供决策支撑数据。
- [重构] **数据传输层优化**：
  - 扩展了 `ParsedBuffItemDTO` 和 `BuffAssetInfo`，实现了双端数据结构对齐，为后续前端展示印花详情铺平道路。

## v1.13.1 (2026-01-08)
- [修复] **印花价值同步任务稳定性与准确性修复**：
  - **异常处理加固**：修复了日志记录中的 f-string 语法错误，彻底解决了在 Pydantic 验证失败时因错误信息包含花括号而触发的 `KeyError: 'appid'` 隐患。
  - **模型解析修正**：重构了 `BuffStickerItem` Pydantic 模型，引入 `AliasPath` 穿透解析 `goods_info.icon_url` 和 `item_type` 嵌套字段，解决了 `ValidationError` 导致的同步中断。
- [优化] **同步效率与数据纯净度提升**：
  - **精准过滤 (API 层)**：通过参数调研，将同步请求从全品类抓取优化为 `category_group=sticker` 定向抓取，单次任务处理量从 2.9 万项缩减至 1.2 万项，显著降低了封号风险并提升了同步速度。
  - **类型校验 (Logic 层)**：在数据持久化前增加了 `item_type` 校验，自动过滤掉“挂件”、“印花板”等非印花物品，确保 `buff_stickers` 表的数据语义纯粹。

## v1.13.0 (2026-01-08)
- [功能] **印花同步系统交互与管控优化**：
  - **交互体验 (UX)**：移除了印花管理页面的“立即同步”按钮，将数据同步逻辑全面收归至“任务配置”中心，实现统一调度。
  - **管控增强 (Backend)**：
    - **唯一性限制**：在 `BuffScanTaskServiceImpl` 中增加了系统任务（商品同步、分类同步、印花同步）的创建校验，确保每种系统任务在全局仅允许存在一个实例。
    - **防误删保护**：在 Service 层实现了 `deleteTask` 逻辑，严格禁止删除系统任务。
  - **配置中心 (Frontend)**：任务配置新增“系统-印花同步”选项，并根据任务类型动态隐藏系统任务的删除操作。
- [修复] **解决后端接口 404/500 异常**：
  - **路径对齐**：修正了前端请求路径与后端 RESTful 路径的不一致问题。
  - **参数解析**：在 Controller 层显式声明了 `@RequestParam` 的 `name` 属性，解决了 Spring Boot 3.x 在反射模式下无法获取参数名导致的问题。

## v1.12.0 (2026-01-08)
- [功能] **全量上线“印花价值同步系统” (Sticker Value Engine)**：
  - **爬虫端 (Crawler)**：
    - 新增 [buff_sticker_spider.py](file:///e:/CodeSpace/PYTHON/niro/niro-spider/spiders/buff_sticker_spider.py)：支持全量爬取 BUFF 印花市场数据，采用 `PostgreSQL upsert` 逻辑确保价格实时更新且数据不重复。
    - 调度集成：在 [task_scanner.py](file:///e:/CodeSpace/PYTHON/niro/niro-spider/spiders/task_scanner.py) 中新增对 `BuffTaskType.SYNC_STICKER` 的支持。
  - **后端 (Backend)**：
    - 完整 CRUD 实现：创建了 `BuffStickerController`、`BuffStickerService` 及其对应的 DTO 和 Mapper。
    - 异步任务触发：通过在 `buff_scan_task` 中注入特殊系统任务，实现了 Java 触发 Python 爬虫的解耦架构。
- [重构] **全局任务类型枚举化与整合**：
  - **Java 端**：将新增的印花同步任务整合进现有的 `TaskTypeEnum`，删除了冗余枚举。统一了 `isSystemTask` 判断逻辑，实现系统任务与用户任务的平滑并存。
  - **Python 端**：引入 `BuffTaskType` 枚举类，其定义（0-4）与 Java 端完全对齐，彻底消除爬虫调度逻辑中的硬编码数字。
  - **前端 (Frontend - 建议)**：
    - 提供了基于 TDesign 的印花管理页面组件设计方案，支持分页展示与手动触发同步。

## v1.11.0 (2026-01-08)
- [架构] **新增“高收益捡漏”底层数据支撑架构**：
  - **多维数据建模**：设计并同步了 Python ([models.py](file:///e:/CodeSpace/PYTHON/niro/niro-spider/storage/models.py)) 与 Java ([entity/](file:///e:/CodeSpace/PYTHON/niro/niro-server/niro-web/src/main/java/com/niro/web/entity/)) 端的数据库模型。
  - **核心模型引入**：
    - `BuffSticker`：支持印花元数据管理，为“印花溢价捡漏”提供算法基石。
    - `BuffGoodsStats`：引入饰品雷达指标（7天均价、流动性评分等），建立动态价格监控锚点。
    - `BuffLeakAlert`：建立捡漏预警审计机制，记录所有触发规则的瞬间及预期收益，支持策略复盘与调优。
  - **一致性对齐**：同步更新了所有相关字段的 ORM 映射、默认值逻辑及时间戳自动更新机制，确保双端业务逻辑的高度一致性。
