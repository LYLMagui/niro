...
create table unbox_record_item (
  id bigint generated always as identity primary key,
  ...
  created_at timestamp not null default now(),
  updated_at timestamp not null default now(),
  constraint uk_unbox_record_item_record_id_sort_no
...
create index idx_unbox_record_user_id_unbox_date
  on unbox_record (user_id, unbox_date desc);

create index idx_unbox_record_goods_id
  on unbox_record (goods_id);

create index idx_unbox_record_item_record_id_sort_no
  on unbox_record_item (record_id, sort_no);

comment on table unbox_record is '开箱记录表';
comment on column unbox_record.id is '主键';
comment on column unbox_record.user_id is '用户id';
comment on column unbox_record.goods_id is '箱子商品id，对应buff_goods表id';
comment on column unbox_record.unbox_date is '开箱日期';
comment on column unbox_record.box_name is '箱子名称';
comment on column unbox_record.default_discount is '默认折扣，取值范围0到1';
comment on column unbox_record.note is '备注';
comment on column unbox_record.created_at is '创建时间';
comment on column unbox_record.updated_at is '更新时间';

comment on table unbox_record_item is '开箱记录明细表';
comment on column unbox_record_item.id is '主键';
comment on column unbox_record_item.record_id is '所属开箱记录id';
comment on column unbox_record_item.sort_no is '记录内顺序号';
comment on column unbox_record_item.handling_status is '处理状态：pending待处理、discarded丢弃、stored暂存、purchased已买';
comment on column unbox_record_item.box_purchase_price is '箱子购入价';
comment on column unbox_record_item.weapon_name is '武器名称';
comment on column unbox_record_item.in_game_price is '游戏内售价';
comment on column unbox_record_item.discount is '明细折扣，为空时继承记录默认折扣';
comment on column unbox_record_item.actual_sell_price is '实际卖出价';
