...
| P4 | 最小权限管理能力 | 提供可操作的分配角色和菜单授权能力 | A10 | 后端管理接口、前端管理页或弹窗 | 进行中（首版已落地） | 4 |
...
| P4-05 | 定义默认角色与初始授权策略 | A03 A10 | P1 P4-03 | 新用户授权行为可预测 | 进行中（沿用 `role_key=user`，待联调验收确认） |
...
- 管理员可通过界面完成角色授权菜单
- 授权变更后重新登录或刷新后立即生效
- 当前进度补充：接口与前端面板已落地，待 JDK21 环境下完成后端编译与联调验收
...
| R7 | 管理端可维护权限关系 | 可完成分配角色与授权菜单 | 进行中 | 首版已落地，待 JDK21 编译与联调验收 |
...
| 2026-03-21 | P4 首版落地 | 新增 RBAC 管理接口与前端权限管理面板（用户分配角色、角色分配菜单），并补齐关联关系替换服务能力 | 是 | `P4-01~P4-04` 首版已完成，前端 `pnpm type-check` 通过 | 本机 Java 为 1.8，后端编译受阻（目标 Java 21），待切换 JDK21 后完成联调验收 |
