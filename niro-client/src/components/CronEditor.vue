...
<template>
  <div class="cron-editor">
    <div class="editor-content">
      <t-tabs v-model="activeTab" theme="normal">
        <t-tab-panel value="second">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="time" size="14px" />秒</div>
          </template>
            ...
            <t-radio-group v-model="second.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: second.type === 'startAt' }]">
                <t-radio value="startAt">
                  ...
                  <t-input-number v-model="second.startAt" :min="0" :max="59" size="small" class="mx-1" />
                  <span class="mx-1">秒开始，每</span>
                  <t-input-number v-model="second.stepValue" :min="1" :max="59" size="small" class="mx-1" />
        ...
        <t-tab-panel value="minute">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="precise-monitor" size="14px" />分</div>
          </template>
          <div class="tab-pane-content">
            <t-radio-group v-model="minute.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: minute.type === 'startAt' }]">
                <t-radio value="startAt">
                  <span class="mr-1">从</span>
                  <t-input-number v-model="minute.startAt" :min="0" :max="59" size="small" class="mx-1" />
                  <span class="mx-1">分开始，每</span>
                  <t-input-number v-model="minute.stepValue" :min="1" :max="59" size="small" class="mx-1" />
        ...
        <t-tab-panel value="hour">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="time-filled" size="14px" />时</div>
          </template>
          <div class="tab-pane-content">
            <t-radio-group v-model="hour.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: hour.type === 'startAt' }]">
                <t-radio value="startAt">
                  <span class="mr-1">从</span>
                  <t-input-number v-model="hour.startAt" :min="0" :max="23" size="small" class="mx-1" />
                  <span class="mx-1">时开始，每</span>
                  <t-input-number v-model="hour.stepValue" :min="1" :max="23" size="small" class="mx-1" />
        ...
        <t-tab-panel value="day">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="calendar" size="14px" />日</div>
          </template>
          <div class="tab-pane-content">
            <t-radio-group v-model="day.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: day.type === 'startAt' }]">
                <t-radio value="startAt">
                  <span class="mr-1">从</span>
                  <t-input-number v-model="day.startAt" :min="1" :max="31" size="small" class="mx-1" />
                  <span class="mx-1">日开始，每</span>
                  <t-input-number v-model="day.stepValue" :min="1" :max="31" size="small" class="mx-1" />
        ...
        <t-tab-panel value="month">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="view-list" size="14px" />月</div>
          </template>
          <div class="tab-pane-content">
            <t-radio-group v-model="month.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: month.type === 'startAt' }]">
                <t-radio value="startAt">
                  <span class="mr-1">从</span>
                  <t-input-number v-model="month.startAt" :min="1" :max="12" size="small" class="mx-1" />
                  <span class="mx-1">月开始，每</span>
                  <t-input-number v-model="month.stepValue" :min="1" :max="12" size="small" class="mx-1" />
        ...
        <t-tab-panel value="week">
          <template #label>
            <div class="flex items-center gap-1.5"><t-icon name="view-module" size="14px" />周</div>
          </template>
          <div class="tab-pane-content">
            <t-radio-group v-model="week.type" direction="vertical">
              ...
              <div :class="['radio-item-wrapper', { active: week.type === 'range' }]">
                <t-radio value="range">
                  ...
                  <t-select v-model="week.rangeStart" size="small" class="mx-1 w-20">
                    <t-option v-for="w in weekOptions" :key="w.value" :value="w.value" :label="w.label" />
                  </t-select>
                  <span class="mx-1">-</span>
              ...
              <div :class="['radio-item-wrapper', { active: week.type === 'startAt' }]">
                <t-radio value="startAt">
                  ...
                  <t-input-number v-model="week.stepValue" :min="1" :max="4" size="small" class="mx-1" />
      ...
      <div class="preview-section mt-1 px-5 py-3">
        ...
        <div class="cron-string-box rounded-lg px-4 py-3 flex items-center justify-between shadow-sm mt-4">
          <div class="flex flex-col">
            <div class="flex items-center gap-2 mb-1">
              <span class="text-[10px] text-blue-400 font-bold uppercase tracking-widest">Cron Expression</span>
              <t-tag v-if="!cronString || cronString === '* * * * * ?'" theme="success" variant="light" size="small" class="px-1.5 h-4 text-[10px]">立即启动</t-tag>
            </div>
            <div class="flex items-center gap-2">
              <span class="font-mono text-base font-bold text-blue-700 tracking-tight">
                {{ (!cronString || cronString === '* * * * * ?') ? '立即启动' : cronString }}
              </span>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <t-button variant="text" shape="square" size="small" @click="handleCopy" :disabled="!cronString || cronString === '* * * * * ?'" class="hover:bg-blue-50">
              <t-icon name="copy" :class="(!cronString || cronString === '* * * * * ?') ? 'text-gray-300' : 'text-blue-500'" />
            </t-button>
            <t-button variant="outline" theme="default" size="small" @click="handleReset" class="reset-btn">
              <t-icon name="refresh" />
              重置
            </t-button>
          </div>
        ...
        <div class="execution-list-container rounded-lg border border-gray-100 bg-gray-50/50 p-2">
          <ul v-if="!isImmediate && nextExecutions.length" class="space-y-1 text-[12px] text-gray-600">
            <li v-for="(time, index) in nextExecutions" :key="index" class="execution-item">
              <span class="font-mono">{{ time }}</span>
            </li>
          </ul>
          <div v-else class="text-[12px] text-gray-400 py-2 text-center italic">
            {{ isImmediate ? '任务将立即启动，不设定时循环' : (nextExecutionsError || '点击"刷新预览"获取未来运行时间点') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 底部按钮 -->
    <div class="flex justify-end items-center gap-2 px-5 py-2 border-t border-gray-50 bg-gray-50/30">
...
const secondOptions = Array.from({ length: 60 }, (_, i) => ({ label: i < 10 ? `0${i}` : `${i}`, value: i }));
const minuteOptions = Array.from({ length: 60 }, (_, i) => ({ label: i < 10 ? `0${i}` : `${i}`, value: i }));
const hourOptions = Array.from({ length: 24 }, (_, i) => ({ label: i < 10 ? `0${i}` : `${i}`, value: i }));
...
const formatUnit = (state: any, unit: string, noneSymbol = '*') => { 
  ...
  switch (type) { 
    case 'every': return '*'; 
    case 'none': return noneSymbol; 
    case 'startAt': 
      return `${startAt}/${stepValue}`; 
    case 'range': return `${rangeStart}-${rangeEnd}`; 
    case 'specific': 
      if (specific.length) return [...specific].sort((a, b) => {
        if (typeof a === 'string' && typeof b === 'string') return a.localeCompare(b);
        return (a as number) - (b as number);
      }).join(',');
      // 如果是特定点但没选，时间单位回退到 0，日期单位回退到 1，月回退到 1
      return isTime ? '0' : (isDayOrMonth ? '1' : '*');
    case 'last': return unit === 'day' ? 'last' : `last ${lastValue}`; 
    default: return '*'; 
  } 
};
...
const cronString = computed(() => {
  ...
  // 判断是否为每秒执行 (* * * * * ?)
  const isEverySecond = second === '*' && minute === '*' && hour === '*' && day === '*' && month === '*' && week === '?';
...
const calcNextExecutions = () => { 
  try { 
    ...
    nextExecutionsError.value = `表达式无效: ${e.message?.split('\n')[0] || e}`; 
  } 
}; 
...
const executeParse = (expression: string) => { 
  try {
    // 兼容处理：将可能存在的 ? 替换为 * (APScheduler 规范) 
    // 将 APScheduler 的 last 替换为 cron-parser 识别的 L，以便预览正常工作
    const safeExpression = expression
      .replace(/\?/g, '*')
      .replace(/last\s+(\w+)/g, '$1L') // last sun -> sunL
      .replace(/last/g, 'L');         // last -> L
    ...
    // 如果是 6 位或以上，必须告诉 cron-parser 第一位是秒 
    if (parts.length >= 6) { 
      options.hasSeconds = true; 
    } 

    // 使用直接调用方式 (参考用户提供的成功案例)
    ...
    for (let i = 0; i < 5; i++) { 
      try {
        ...
        // 兼容处理：有些版本返回 Date，有些返回 { value: CronDate }
...
const parseCron = (cron: string) => {
  ...
  if (!cron || cron === '* * * * * ?' || cron === '* * * * * *') {
    ...
    setTimeout(() => { ignoreWatch.value = false; }, 0);
    return;
  }
  isImmediate.value = false;
...
.cron-string-box {
  ...
  border: 1px border-dashed #bfdbfe;
}
