 ...
 */
export function useUiState(formData: { taskType: number; cronExpression: string; platform: string; scanIntervalMin: number; scanIntervalMax: number; scanInterval: number | undefined; durationMinutes: number; restPeriod: number }) {
     ...
     * 自动清除换行符和首尾空格
     */
    const handleInputTrim = (val: string | InputEvent, target: Record<string, string>, key: string) => {
