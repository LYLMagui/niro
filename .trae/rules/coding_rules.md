...
本项目 Python 代码规范：
- 代码格式和项目结构参考企业级项目
- 所有的 Python 代码都必须在`niro-spider`模块中实现，禁止在其他模块中实现
- 适当引入第三方库来提高代码的规范性、可扩展性、简洁性和安全性
- 引入的第三方库必须是主流的，社区活跃的，有良好的文档和维护团队
- 编写代码时，优先使用成熟的第三方库，而不是自己实现相同功能
- 代码注释必须要清晰，包括函数、类、模块的注释，注释必须要符合 PEP 8 规范
...
**非常重要**：
1. release.md只需要在提交代码的时候更新一次，只需要简洁记录每次迭代的改动内容
2. 不管是前端，后端，还是python端的代码中，禁止使用‘魔法值’，尽量抽离出为枚举类或者常量类，比较通用的可以创建一个公共常量类