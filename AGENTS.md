...
4. **参数强制要求**：调用 Codex MCP 时必须使用 `model: "gpt-5.4"`、`sandbox: "danger-full-access"`、`approval-policy: "on-failure"` —— 没有例外
...
| 架构设计 | **CC** | 纯设计决策 | 规划能力强 |
| collab.no_plan | 禁止生成任务计划、优先级排序、技术方案选型，这些由主AI通过 shrimp-task-manager MCP完成 | 职责边界 |
...
**必须始终包含 `model: "gpt-5.4"`、`sandbox: "danger-full-access"`、`approval-policy: "on-failure"`** —— 不得违背
...
- **Codex（默认）**：所有代码相关任务 → 使用结构化 prompt 调用，**必须带 `model: "gpt-5.4"`、`sandbox: "danger-full-access"`、`approval-policy: "on-failure"`**，保存 conversationId 并跟进
...
| 反模式 | 问题 | 解决方案 |
|--------|------|----------|
| **使用错误的模型** | **严重错误：未使用 gpt-5.4** | **必须始终使用 `model: "gpt-5.4"` —— 无例外** |
...
- **智能工具策略**（C:\Users\24160\.Codex\workflows\intelligent-tools-strategy.md）：在调用组合工具前先确认上下文注入顺序与冲突处理方式。
- **上下文搜索命令**（C:\Users\24160\.Codex\workflows\context-search-strategy.md）：按照既定查询模板管理检索结果，并回写引用来源。

### Codex Collaboration Skill 使用指南

这是一个skill（技能），不是agent代理。使用方式：`使用 codex-collaboration skill 进行[具体任务]`
...
Codex 会根据任务复杂度自动识别何时需要使用该 skill，无需手动指定。
